...
class SeatsSection extends Component {
    ...
    {
        //		let sect = this.props.stmp.sections;

        let seatsHtml = [];


        for (let s = 0; s < this.props.stmp.sections.length; s++) {
            ...
            /*	const layout = [ {pos:'W',name:'A'},
                              ...
                              {pos:'W',name:'J'}]
                              */
            let layout = this.props.stmp.sections[s].layout;
            let start = this.props.stmp.sections[s].start;
            let finish = this.props.stmp.sections[s].end;
            ...
            for (let i = start; i < finish; i++) {
                ...
                for (let x = 0; x < layout.length; x++) {
                    ...
                    colsHtml.push( < SeatButton ailseClass = {
                            el
                        }
                        rowId = {
                            i
                    ...
                    seatsHtml.push( < div className = "row" > {
                            colsHtml
                        } < /div>);
